...
function Topbar({ state, onOpenOverflow, overflowOpen, onOpenActivate, activateOpen }: TopbarProps) {
  ...
  const inTest = state.mode === 'test-idle' || state.mode === 'test-running' || state.mode === 'test-done' || state.mode === 'test-diff';
  ...
  return (
    <header className={styles.topbar}>
      ...
      <div className={styles.crumb}>
        ...
        {inTest ? (
          <span className={styles.crumbTestMarker}>
            <span className={styles.crumbTestPill}>Testing</span>
            <span className={styles.crumbTestName}>
              {activeScenario?.name ?? state.playbook.frontmatter.name}
            </span>
          </span>
        ) : (
          <>
      ...
      ) : state.mode === 'test-diff' ? (
        <button className={`${styles.testBtn} ${styles.testBtnPrimary}`} onClick={state.exitDiff} type="button">
...
/* ============================================================ */
type RowView = 'json' | 'preview' | 'split';
...
function TracePanel({
  ...
  useEffect(() => { if (!state.trace.length) { setExpandedIdx(null); setRowViews({}); } }, [state.trace.length]);

  // Switch automatically to History when we have runs but no live trace
  useEffect(() => {
    if (state.mode === 'test-idle' && state.activeScenarioId) {
      const runs = state.runs[state.activeScenarioId] || [];
      if (runs.length > 0 && state.trace.length === 0) {
        // leave tab as-is; user can switch manually
      }
    }
  }, [state.mode, state.activeScenarioId, state.runs, state.trace.length]);

  // Diff mode renders a completely different layout
  if (state.mode === 'test-diff' && state.diffPair) {
    return <DiffTracePanel state={state} />;
  }

  const activeRuns = state.activeScenarioId ? (state.runs[state.activeScenarioId] ?? []) : [];
    ...
    <section className={styles.tracePanel}>
      <header className={styles.traceHead}>
        <div className={styles.traceTabs}>
          {(['trace', 'variables', 'history'] as const).map((t) => (
            <button
              key={t}
              className={`${styles.traceTab} ${tab === t ? styles.traceTabActive : ''}`}
              onClick={() => setTab(t)}
              type="button"
            >
              {t === 'trace' ? 'Trace' : t === 'variables' ? 'Variables' : 'Run history'}
              {t === 'history' && activeRuns.length > 0 && (
                <span className={styles.traceTabV2Tag}>{activeRuns.length}</span>
              )}
            </button>
          ))}
        </div>
        <span className={styles.traceMeta}>
          ...
          {tab === 'history' && <span>{activeRuns.length} run{activeRuns.length === 1 ? '' : 's'}</span>}
        </span>
      </header>
      <div className={styles.traceList}>
        {tab === 'trace' && (
          <>
            {!state.trace.length && (
              <div className={styles.traceEmpty}>No steps run yet. Click <strong>Run test</strong> in the rail to start.</div>
            )}
            {state.trace.map((entry, i) => {
              ...
              const view: RowView = rowViews[i] ?? 'split';
              const formatPretty = (v?: string) => {
                if (!v) return '';
                try { return JSON.stringify(JSON.parse(v), null, 2); } catch { return v; }
              };
              const setView = (v: RowView) => setRowViews((p) => ({ ...p, [i]: v }));
              return (
                <div key={i} className={`${styles.traceRowWrap} ${isExpanded ? styles.traceRowWrapOpen : ''}`}>
                  ...
                  {isExpanded && (
                    <div className={`${styles.traceRowDetail} ${view !== 'split' ? styles.traceRowDetailJsonOnly : ''}`}>
                      <div className={styles.traceDetailHeader}>
                        <span className={styles.traceDetailLabel}>View</span>
                        <span
                        ...
                        >
                          <button
                            type="button"
                            className={`${styles.traceViewToggleBtn} ${view === 'json' ? styles.traceViewToggleBtnActive : ''}`}
                            onClick={() => setView('json')}
                          >JSON</button>
                          <button
                            type="button"
                            className={`${styles.traceViewToggleBtn} ${view === 'preview' ? styles.traceViewToggleBtnActive : ''}`}
                            onClick={() => setView('preview')}
                          >Preview</button>
                          <button
                            type="button"
                            className={`${styles.traceViewToggleBtn} ${view === 'split' ? styles.traceViewToggleBtnActive : ''}`}
                            onClick={() => setView('split')}
                          >Split</button>
                        </span>
                      </div>
                      {(view === 'json' || view === 'split') && (
                        <>
                      ...
                      {(view === 'preview' || view === 'split') && (
                        <div className={styles.traceDetailBlock}>
                          <div className={styles.traceDetailLabel}>Artifact</div>
                          <ArtifactPane chip={chip} entry={entry} />
                        </div>
                      )}
                      {entry.errorMessage && (
                        <div className={styles.traceDetailBlock}>
        ...
        {tab === 'history' && <HistoryTab state={state} runs={activeRuns} />}
      </div>
    </section>
  );
}
...
function HistoryTab({
  ...
  state: ReturnType<typeof useCanvasState>;
  runs: Array<{ id: string; startedAt: number; endedAt: number; outcome: 'pass' | 'fail' | 'cancelled'; versionHash: string }>;
}) {
  if (!runs.length) {
    return <div className={styles.historyEmpty}>No runs yet for this scenario. Click Run test to start a history.</div>;
  }
  const fmt = (t: number) => {
    const d = new Date(t);
    const now = Date.now();
  ...
  return (
    <>
      {runs.map((run, idx) => {
        ...
        const outcomeClass =
          run.outcome === 'pass' ? styles.historyRowOutcomePass :
          run.outcome === 'fail' ? styles.historyRowOutcomeFail :
          styles.historyRowOutcomeCancelled;
        const compareAgainst = runs[idx === 0 ? 1 : 0];
        return (
          <button
            key={run.id}
            className={styles.historyRow}
            onClick={() => { /* load this run into Trace tab — future enhancement */ }}
            type="button"
          >
            <span className={styles.historyRowTime}>{fmt(run.startedAt)}</span>
            <span className={styles.historyRowVer}>{run.versionHash}</span>
            <span className={`${styles.historyRowOutcome} ${outcomeClass}`}>{run.outcome}</span>
            <span className={styles.historyRowDur}>{dur}ms</span>
            <span
              className={styles.historyRowCompare}
              role="button"
              tabIndex={0}
              onClick={(e) => {
                e.stopPropagation();
                if (compareAgainst) state.enterDiff(run.id, compareAgainst.id);
                else window.alert('Need another run to compare against.');
              }}
...
function DiffTracePanel({ state }: { state: ReturnType<typeof useCanvasState> }) {
  const pair = state.diffPair!;
  const findRun = (id: string) => {
    for (const arr of Object.values(state.runs)) {
      const found = arr.find((r) => r.id === id);
      if (found) return found;
    }
    return null;
  };
  const a = findRun(pair.aRunId);
  const b = findRun(pair.bRunId);
  if (!a || !b) {
    return (
      <section className={styles.tracePanel}>
        <div className={styles.historyEmpty}>One or both runs could not be loaded.</div>
      </section>
    );
  }
  // Align by stepId. Build a unified ordering using A's order, then append any in B not in A.
  const aIds = a.trace.map((e) => e.stepId);
  const bIds = b.trace.map((e) => e.stepId);
  const orderedIds: string[] = [...aIds];
  for (const id of bIds) if (!orderedIds.includes(id)) orderedIds.push(id);
  const aMap = new Map(a.trace.map((e) => [e.stepId, e]));
  const bMap = new Map(b.trace.map((e) => [e.stepId, e]));
  const branchesDiverged = JSON.stringify(a.branchPath) !== JSON.stringify(b.branchPath);
  const formatTime = (t: number) => new Date(t).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });

  return (
    <section className={styles.tracePanel}>
      <header className={styles.traceHead}>
        <div className={styles.traceTabs}>
          <span className={`${styles.traceTab} ${styles.traceTabActive}`}>Diff</span>
        </div>
        <span className={styles.traceMeta}>
          {orderedIds.length} step{orderedIds.length === 1 ? '' : 's'} compared
          {branchesDiverged && <span className={styles.traceOutcomeErr}> · branches diverged</span>}
        </span>
      </header>
      <div className={styles.diffShell}>
        <div className={styles.diffColHead}>
          Run A &middot; {formatTime(a.startedAt)} &middot;
          <span className={styles.diffColHeadStamp}>{a.versionHash}</span>
        </div>
        <div className={styles.diffColHead}>
          Run B &middot; {formatTime(b.startedAt)} &middot;
          <span className={styles.diffColHeadStamp}>{b.versionHash}</span>
        </div>
      </div>
      <div className={styles.diffShell}>
        <div className={styles.diffCol}>
          {orderedIds.map((id, i) => {
            ...
            if (!e) {
              return (
                <div key={`a-${id}`} className={`${styles.diffStep} ${styles.diffStepMissing}`}>
                  <span className={styles.diffStepNum}>{String(i + 1).padStart(2, '0')}</span>
                  <span>Not run</span>
                  <span />
                </div>
              );
            }
            const outputDiffers = bE && bE.output !== e.output;
            const cls = outputDiffers ? `${styles.diffStep} ${styles.diffStepWarn}` : styles.diffStep;
            return (
              <div key={`a-${id}`} className={cls}>
                <span className={styles.diffStepNum}>{String(i + 1).padStart(2, '0')}</span>
                <span>{e.output || e.status}</span>
                <span className={styles.diffStepDur}>{e.durationMs ? `${e.durationMs}ms` : ''}</span>
              </div>
            );
          })}
        </div>
        <div className={styles.diffCol}>
          {orderedIds.map((id, i) => {
            ...
            if (!e) {
              return (
                <div key={`b-${id}`} className={`${styles.diffStep} ${styles.diffStepMissing}`}>
                  <span className={styles.diffStepNum}>{String(i + 1).padStart(2, '0')}</span>
                  <span>Not run</span>
                  <span />
                </div>
            ...
            const outputDiffers = aE && aE.output !== e.output;
            const cls = outputDiffers ? `${styles.diffStep} ${styles.diffStepWarn}` : styles.diffStep;
            return (
