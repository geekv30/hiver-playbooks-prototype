...
import type { Scenario, Run } from './test-fixtures';
import {
  SEED_SCENARIOS,
  buildSeedRuns,
  versionHash,
  RUNS_PER_SCENARIO_CAP,
  TESTV2_STORAGE_KEY,
} from './test-fixtures';
...
export type TestRailTab = 'scenarios' | 'batch';
...
export interface CanvasState {
  ...
  testRailTab: TestRailTab;
  diffPair: { aRunId: string; bRunId: string } | null;
  pickerOpen: boolean;
}
...
type Action =
  ...
  | { type: 'updateScenarioInput'; id: string; input: Scenario['input'] }
  | { type: 'setScenarioMode'; id: string; mode: Scenario['mode']; input: Scenario['input'] }
  | { type: 'appendRun'; scenarioId: string; run: Run }
  | { type: 'clearRunsForScenario'; scenarioId: string }
  | { type: 'setTestRailTab'; tab: TestRailTab }
  | { type: 'enterDiff'; aRunId: string; bRunId: string }
  | { type: 'exitDiff' }
  | { type: 'setPickerOpen'; open: boolean }
  | { type: 'hydrateTestStore'; store: Partial<Pick<CanvasState, 'scenarios' | 'activeScenarioId' | 'runs'>> };
...
function reducer(state: CanvasState, action: Action): CanvasState {
  switch (action.type) {
    ...
    case 'deleteScenario': {
      ...
      const { [action.id]: _removed, ...restRuns } = state.runs;
      const nextActive = state.activeScenarioId === action.id
        ? (nextScenarios[0]?.id ?? null)
        : state.activeScenarioId;
      return { ...state, scenarios: nextScenarios, runs: restRuns, activeScenarioId: nextActive };
    }
    case 'selectScenario':     return { ...state, activeScenarioId: action.id };
    case 'updateScenarioInput': {
      return {
        ...state,
        scenarios: state.scenarios.map((s) =>
          s.id === action.id ? { ...s, input: action.input, updatedAt: Date.now() } : s
        ),
      };
    }
    case 'setScenarioMode': {
      return {
        ...state,
        scenarios: state.scenarios.map((s) =>
          s.id === action.id ? { ...s, mode: action.mode, input: action.input, updatedAt: Date.now() } : s
        ),
      };
    }
    case 'appendRun': {
      const prior = state.runs[action.scenarioId] ?? [];
      const next = [action.run, ...prior].slice(0, RUNS_PER_SCENARIO_CAP);
      return { ...state, runs: { ...state.runs, [action.scenarioId]: next } };
    }
    case 'clearRunsForScenario': {
      const { [action.scenarioId]: _r, ...rest } = state.runs;
      return { ...state, runs: rest };
    }
    case 'setTestRailTab':     return { ...state, testRailTab: action.tab };
    case 'enterDiff':          return { ...state, mode: 'test-diff', diffPair: { aRunId: action.aRunId, bRunId: action.bRunId } };
    case 'exitDiff':           return { ...state, mode: 'test-done', diffPair: null };
    case 'setPickerOpen':      return { ...state, pickerOpen: action.open };
    case 'hydrateTestStore': {
      return {
        ...state,
        scenarios: action.store.scenarios ?? state.scenarios,
        activeScenarioId: action.store.activeScenarioId !== undefined ? action.store.activeScenarioId : state.activeScenarioId,
        runs: action.store.runs ?? state.runs,
      };
    }
...
/** Build a fresh initial state. Seed-scenario runs use the current playbook's
 *  hash so the first run on first load is "same version" — diff demo across
 *  versions only makes sense after the user edits + reruns.
 */
function buildInitialState(): CanvasState {
  const v = versionHash(WALK_JAPAN_SEED);
  // Seed runs reference two version hashes so the diff demo has variety:
  // versionA = current, versionB = "3 days ago" (we cheat and reuse current
  // for both so the data is consistent on first load).
  const seedRuns = buildSeedRuns(v, v);
  return {
    ...
    runs: seedRuns,
    testRailTab: 'scenarios',
    diffPair: null,
    pickerOpen: false,
  };
}

const INITIAL_STATE: CanvasState = buildInitialState();
...
export function useCanvasState() {
  ...
  // Accumulators for the in-flight run so we can persist it on completion.
  const runAccumRef = useRef<{ trace: TraceEntry[]; branchPath: string[]; startedAt: number } | null>(null);

  const persistRun = useCallback((outcome: 'pass' | 'fail' | 'cancelled') => {
    const accum = runAccumRef.current;
    const scenarioId = state.activeScenarioId;
    if (!accum || !scenarioId) return;
    const run: Run = {
      id: `run-${Date.now()}`,
      scenarioId,
      ...
      endedAt: Date.now(),
      outcome,
      versionHash: versionHash(state.playbook),
      trace: accum.trace,
      variables: {},
      branchPath: accum.branchPath,
    };
    dispatch({ type: 'appendRun', scenarioId, run });
    runAccumRef.current = null;
  }, [state.activeScenarioId, state.playbook]);
  ...
  const runTest = useCallback(() => {
    ...
    runAccumRef.current = { trace: [], branchPath: [], startedAt: Date.now() };
    ...
    const collect = (steps: AnyStep[]) => {
      for (const s of steps) {
        if (s.kind === 'action') {
          ...
          if (yes) {
            if (runAccumRef.current) runAccumRef.current.branchPath.push(`${s.id}:${yes.id}`);
            collect(yes.steps);
          }
          if (no) {
    ...
    const tick = () => {
      ...
      if (cur.isElseSkipped) {
        ...
        if (runAccumRef.current) runAccumRef.current.trace.push(entry);
        i++;
  ...
  }, [state.playbook, state.playbook.steps, persistRun]);
  ...
  /* ---- testv2 dispatchers ---- */
  const createScenario = useCallback((partial: { name: string; mode: Scenario['mode']; input: Scenario['input'] }) => {
    const now = Date.now();
    ...
    dispatch({ type: 'createScenario', scenario });
  }, []);

  const renameScenario = useCallback((id: string, name: string) => dispatch({ type: 'renameScenario', id, name }), []);
  const deleteScenario = useCallback((id: string) => dispatch({ type: 'deleteScenario', id }), []);
  const selectScenario = useCallback((id: string | null) => dispatch({ type: 'selectScenario', id }), []);
  const updateScenarioInput = useCallback((id: string, input: Scenario['input']) => dispatch({ type: 'updateScenarioInput', id, input }), []);
  const setScenarioMode = useCallback((id: string, mode: Scenario['mode'], input: Scenario['input']) => dispatch({ type: 'setScenarioMode', id, mode, input }), []);
  const clearRunsForScenario = useCallback((scenarioId: string) => dispatch({ type: 'clearRunsForScenario', scenarioId }), []);
  const setTestRailTab = useCallback((tab: TestRailTab) => dispatch({ type: 'setTestRailTab', tab }), []);
  const enterDiff = useCallback((aRunId: string, bRunId: string) => dispatch({ type: 'enterDiff', aRunId, bRunId }), []);
  const exitDiff = useCallback(() => dispatch({ type: 'exitDiff' }), []);
  const setPickerOpen = useCallback((open: boolean) => dispatch({ type: 'setPickerOpen', open }), []);

  /* ---- localStorage persistence (scenarios + runs only; playbook stays in memory) ---- */
  const hydratedRef = useRef(false);
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      const raw = window.localStorage.getItem(TESTV2_STORAGE_KEY);
      if (raw) {
        const parsed = JSON.parse(raw) as Partial<Pick<CanvasState, 'scenarios' | 'activeScenarioId' | 'runs'>>;
        dispatch({ type: 'hydrateTestStore', store: parsed });
      }
    } catch {
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.localStorage.setItem(TESTV2_STORAGE_KEY, payload);
    } catch {
      // ignore quota errors
    }
  }, [state.scenarios, state.activeScenarioId, state.runs]);

  const currentVersionHash = useMemo(() => versionHash(state.playbook), [state.playbook]);
